<template>
    <section data-aos="fade-up" data-aos-duration="3000" class="min-h-screen bg-cover" style="
                            ...
                            <form @submit.prevent="submitForm" class="mt-6">
                                ...
                                <button
                                    ...
                                    <input type="button" value="Send Message"
                                        onClick="alert( 'message received', );window.location='/';">
                                </button>
                            </form>
                        </div>
                    </div>
...
export default {
    ...
    methods: {
        async submitForm() {
            const supabaseUrl = "https://tikrivdbqwmjfuharvnw.supabase.co";
            ...
            const supabaseClient = supabase.fromURL(sudabaseUrl, sudabaseKey);
            const formData = { name: this.name, email: this.email, message: this.message };
            try {
                const { body } = await supabaseClient.post("/users", formData);
